...
export default function IndexEntity<T>(props: indexEntityProps<T>) {
    ...
    const buttons = (editUrl: string, id: number) => <>
        ...
        <Link className="btn btn-primary waves-effect waves-light"
            to={editUrl}>Editar</Link>

        <Button
            onClick={() => customConfirm(() => deleteEntity(id))}
            className="btn btn-danger bg-gradient waves-effect waves-light">Eliminar</Button>
    </>
    ...
    return (
        <>
            ...
            {props.createURL ? <Link className="btn btn-primary btn-label waves-effect right waves-light"
            to={props.createURL}><i className="ri-user-smile-line label-icon align-middle fs-16 ms-2"></i>Crear {props.entityName}</Link> : null}
